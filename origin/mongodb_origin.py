...
# 需要排序
PRE_DEFINE_DATE_FIELDS = ['date', 'trade_date', 'ann_date', 'in_date', 'datetime']
...
class MongodbOrigin(DataOrigin):
    ...
    def props_to_mongo(self, props):
        ...
        view = props.get('view')
        date_field_name = props.pop('date_field', None)
        symbol_field_name = props.pop('symbol_field', None)
        ...
        if date_field_name:
            assert date_field_name in exist_fields, 'date_field in props is not exist!'
            date_field_type = type(doc[exist_fields])

        else:
            date_fields = [i for i in exist_fields if 'date' in i.lower()]

            assert date_field_name in exist_fields, 'date_field in props is not exist!'

            if len(date_fields) >= 1:
                for field in PRE_DEFINE_DATE_FIELDS:
                    if field in exist_fields:
                        date_field_name = field
                        date_field_type = type(doc[field])
                        break
            else:
                date_field_name = None
                date_field_type = None

        proj = {'_id': 0, date_field_name: 1}

        if date_field_type == int:
            flt = {date_field_name: {'$gte': int(start_date), '$lte': int(end_date)}}
        elif date_field_type == str:
            flt = {date_field_name: {'$gte': str(start_date), '$lte': str(end_date)}}
        elif 'datetime' in date_field_name.lower():
            flt = {date_field_name: {'$gte': datetime.strptime(str(start_date), '%Y%m%d'),
                                     '$lte': datetime.strptime(str(end_date), '%Y%m%d')}}
        else:
            flt = {}

        proj.update({date_field_name: 1}) if date_field_name is not None else None
