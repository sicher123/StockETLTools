...
class DailyDB(object):
    ...
    def _update_flag(self, data, name, h5_obj=None):
        ...
        if h5_obj is None:
            colname = data.name
            file = self.get_file(colname)
            file.create_dataset(name, data=flag, chunks=True,
                                maxshape=(len(data)*3, 1), compression="gzip")
            ...
            h5_obj.create_dataset(name, data=flag,
                                  maxshape=(len(data)*3, 1), chunks=True)
            return flag
    ...
    def _create_a_file(self, data, field):
        if field not in self.exist_fields:
            ...
            index_num, column_num = data.shape
            file.create_dataset('data', data=dt, chunks=True,
                                maxshape=(index_num*3, column_num*3),
                                compression="gzip")
